...
@section('content_header')
    <h1><strong>Catálogo / Cartera de Servicios</strong> <small>C.E.C.O.S.A.M.A.</small></h1>
@stop
...
<div class="card">
    ...
    <form action="{{ route('updateCecosama', $profesionalOcupaciones->id) }}" method="POST">
        @csrf 
        @method('PUT')
                    ...
                    <select name="ocupacion" id="ocupacion" class="form-control select2">
                        ...
                        @foreach($ocupaciones as $ocupacion)
                            <option value="{{ $ocupacion->id }}" 
                                {{ old('ocupacion', optional($profesionalOcupaciones)->id_catalogo) == $ocupacion->id ? 'selected' : '' }}>
