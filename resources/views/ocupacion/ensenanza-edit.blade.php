...
@section('content_header')
    <h1><strong>Catálogo / Cartera de Servicios</strong> <small>Pasantes Médicos / Enfermería</small></h1>
@stop
...
<div class="card">
    ...
    <form action="{{ route('updateEnsenanza', $profesionalOcupaciones->id) }}" method="POST">
        @csrf 
        @method('PUT')
                    ...
                    <select name="ocupacion_uno" id="ocupacion_uno" class="form-control select2">
                        ...
                        @foreach($ocupaciones as $ocupacion)
                            <option value="{{ $ocupacion->id }}" 
                                {{ old('ocupacion_uno', optional($profesionalOcupaciones)->id_catalogo) == $ocupacion->id ? 'selected' : '' }}>
