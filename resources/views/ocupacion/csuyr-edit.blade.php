...
@section('content_header')
    <h1><strong>Ocupación / Cartera de Servicios</strong> <small>Centro de Salud Urbano y Rural</small></h1>
@stop
...
<div class="card">
    ...
    <form action="{{ route('updateCentrosDeSalud', $profesionalOcupaciones->id) }}" method="POST">
        @csrf 
        @method('PUT')
                    ...
                    <select name="ocupacion_uno" id="ocupacion_uno" class="form-control select2">
                        ...
                        @foreach($ocupaciones as $ocupacion)
                            <option value="{{ $ocupacion->id }}" 
                                {{ old('ocupacion_uno', $profesionalOcupaciones->id_catalogo_uno) == $ocupacion->id ? 'selected' : '' }}>
            ...
            <div class="row mt-3">
                <div class="col-md-12">
                    ...
                    <select name="ocupacion_dos" id="ocupacion_dos" class="form-control select2">
                        ...
                        @foreach($ocupaciones as $ocupacion)
                            <option value="{{ $ocupacion->id }}" 
                                {{ old('ocupacion_dos', $profesionalOcupaciones->id_catalogo_dos) == $ocupacion->id ? 'selected' : '' }}>
                                {{ $ocupacion->unidad }} - {{ $ocupacion->area }} - {{ $ocupacion->subarea }} - {{ $ocupacion->ocupacion }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="card-footer">
            <button type="submit" class="btn btn-success btn-sm">ACTUALIZAR DATOS DE OCUPACIÓN</button>
        </div>
    </form>
</div>
